import {
  cloneElement,
  useContext,
  useLayoutEffect,
  useRef,
  type ReactElement,
  type ReactNode,
} from "react";

import { useGSAP } from "@gsap/react";
import gsap from "gsap";
import { Draggable } from "gsap/Draggable";

import WindowContext from "./windowContext";
import type { WindowKey } from "@store/window";
import useWindowStore from "@store/window";

gsap.registerPlugin(useGSAP, Draggable);

function useWindowContext() {
  const context = useContext(WindowContext);

  if (!context)
    throw new Error("Window component must be used within <Window>");

  return context;
}

function Window({
  children,
  windowKey,
}: {
  children: ReactNode;
  windowKey: WindowKey;
}) {
  const state = useWindowStore();
  return (
    <WindowContext.Provider value={{ state, windowKey }}>
      {children}
    </WindowContext.Provider>
  );
}

function Wrapper({ children }: { children: ReactNode }) {
  const {
    state: { windows, focusWindow },
    windowKey,
  } = useWindowContext();
  const { isOpen, zIndex } = windows[windowKey];
  const ref = useRef<HTMLElement>(null);

  useGSAP(() => {
    const el = ref.current;
    if (!el) return;
    el.style.display = "block";

    gsap.fromTo(
      el,
      { scale: 0.8, opacity: 0, y: 40 },
      { scale: 1, opacity: 1, y: 0, duration: 0.2, ease: "power3.out" },
    );
  }, [isOpen]);

  useGSAP(() => {
    const el = ref.current;
    if (!el) return;

    const [instance] = Draggable.create(el, {
      onPress: () => focusWindow(windowKey),
    });

    return () => instance.kill();
  });

  useLayoutEffect(() => {
    const el = ref.current;
    if (!el) return;

    el.style.display = isOpen ? "block" : "none";
  }, [isOpen]);

  return (
    <section ref={ref} id={windowKey} style={{ zIndex }} className="absolute">
      {children}
    </section>
  );
}

function Close({
  children,
}: {
  children: ReactElement<{ onClick?: () => void }>;
}) {
  const {
    state: { closeWindow },
    windowKey,
  } = useWindowContext();
  return cloneElement(children, { onClick: () => closeWindow(windowKey) });
}

Window.Wrapper = Wrapper;
Window.Close = Close;

export default Window;
